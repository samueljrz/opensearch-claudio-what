...
import (
	...
	"github.com/vtex/opentelemetry-collector/exporter/opensearchexporter/services"
...
type Config struct {
	...
	// VTEX Evidence index Open Search cluster endpoint
	EvidenceEndpoints []string `mapstructure:"evidence_endpoints"`
	...
	WAL                WALSettings       `mapstructure:"wal"`
}
...
type WALSettings struct {
	...
	// S3 Bucket where the WAL log files will be added.
	S3Bucket string `mapstructure:"s3_bucket"`

	// WAL flushing options.
	Flush FlushSettings `mapstructure:"flush"`

	// The cluster where the collector is running.
	Cluster string `mapstructure:"cluster"`

	// Enable or disable the WAL.
	Enabled bool `mapstructure:"enabled"`

	UseMinio bool `mapstructure:"use_minio"`

	Minio services.MinioConfig `mapstructure:"minio"`

	// Send by WAL only. Used when we need to send data only by WAL.
	SendByWalOnly bool `mapstructure:"send_by_wal_only"`
}
